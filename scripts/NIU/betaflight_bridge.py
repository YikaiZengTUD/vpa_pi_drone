...
class BetaflightBridge:
    ...
    def check_bf_status(self):
        try:
            self.board.getData(MultiWii.STATUS)
            status_data = getattr(self.board, 'status', {})
            if status_data:
                arm_status = status_data.get('armed', None)
                failsafe_status = status_data.get('failsafe', None)
                sensors_status = status_data.get('sensors', None)
                rospy.loginfo(f"BF STATUS - Armed: {arm_status}, Failsafe: {failsafe_status}, Sensors: {sensors_status}")
            else:
                rospy.logwarn("No STATUS data received from flight controller")
        except Exception as e:
            rospy.logwarn(f"STATUS read failed: {e}")
