    ...
    def receiveDataPacket(self,terminated=False):
        ...
        with self.serial_port_read_lock:
            ...
            if code == MultiWii.ATTITUDE:
                ...
                # Accept 10/11 bytes, and handle concatenated frames (20/22/33...)
                def _parse_chunk(ch):
                    if len(ch) >= 11:
                        ct,i2c,sens,flags,currentSet = struct.unpack('<HHHIb', ch[:11])
                    elif len(ch) == 10:
                        ct,i2c,sens,flags = struct.unpack('<HHHI', ch)
                        currentSet = 0
                    else:
                        return None
                    return dict(cycleTime=ct, i2c_errors_count=i2c, sensor=sens,
                                flag=flags, **{'global_conf.currentSet': currentSet})

                out = None
                step = 11 if (datalength % 11) == 0 else (10 if (datalength % 10) == 0 else 0)
                if step:
                    # take the **last** (most recent) chunk
                    last = data[-step:]
                    parsed = _parse_chunk(last)
                    if parsed:
                        self.status.update(parsed)
                        self.status['timestamp'] = readTime
                        return self.status
                print(f"STATUS unexpected len={datalength}, dropping")
                return None
